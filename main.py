...
class DOPS:
    ...
    def _actor_loss(self, state: torch.Tensor) -> Tuple[torch.Tensor, float, float]:
        ...
        # q_value_min = q_value_dist.mean(0)
    ...
    def _critic_diversity_loss(
        self, state: torch.Tensor, action: torch.Tensor, require_squre: bool = False,
    ) -> torch.Tensor:
        ...
        q_action_grad_mean = torch.mean(q_action_grad, 1, True)
        q_action_grad_mean = q_action_grad_mean.repeat(1, num_critics, 1)
        q_action_grad = q_action_grad - q_action_grad_mean
        ...
        if require_squre:
            q_action_grad = torch.square(q_action_grad)
    ...
    def _critic_loss(
        ...
        # piu_std = 0.1 * torch.rand_like(action)
        # piu_action = torch.normal(action, piu_std)
        with torch.no_grad():
            current_action, current_action_log_prob = self.actor(state, need_log_prob=True)
            ...
            #q_next = self.target_critic(next_state, next_action).mean(0)
        ...
        q_current = self.critic(state, current_action)
        q_current = q_current - self.alpha * current_action_log_prob
        ql_loss = (torch.clamp(q_current - q_values, 0.5, 100)).mean(dim=1).sum(dim=0)

        q_piu = self.critic(state, piu_action)
        my_loss = (torch.clamp(q_piu - q_values, 0.1, 10)).mean(dim=1).sum(dim=0)
        # cat_action = torch.cat([action, current_action, next_action], dim=0)
        # cat_state = torch.cat([state, state, next_state], dim=0)
        dataset_diversity_loss = self._critic_diversity_loss(state, action)
        pi_diversity_loss = self._critic_diversity_loss(state, current_action)

        loss = critic_loss - self.eta * dataset_diversity_loss - self.eta * pi_diversity_loss + ql_loss + my_loss
        return loss
